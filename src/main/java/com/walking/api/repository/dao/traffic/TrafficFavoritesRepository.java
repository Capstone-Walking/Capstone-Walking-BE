...
public interface TrafficFavoritesRepository extends JpaRepository<TrafficFavoritesEntity, Long> {
	...
	List<TrafficFavoritesEntity> findByMemberFkAndDeletedFalse(Long memberFk);

	Optional<TrafficFavoritesEntity> findByIdAndDeletedFalse(Long id);

	Optional<TrafficFavoritesEntity> findByIdAndMemberFkAndDeletedFalse(Long id, Long memberFk);
}
